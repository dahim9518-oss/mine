from Data_Base import get_connection
import sqlite3

# اوبجكت فيه كل معلومات المادة
class CourseData:
    def __init__(self, code, name, credits, lecture_houre, lap_hour, capacity, prerequisites):
        self.code = code
        self.name = name
        self.credits = credits  # (تم التصحيح: credits بدل creditis)
        self.lecture_houre = lecture_houre
        self.lap_hour = lap_hour
        self.capacity = capacity
        self.prerequisites = prerequisites

# اوبجكت فيه معلومات البلوك الزمني للمادة
class TimeBlock:
    def __init__(self, course_code, day, start_tim, end_tim):
        self.course_code = course_code
        self.day = day
        self.start_tim = start_tim
        self.end_tim = end_tim

class RegistrationSystem: 
    def __init__(self): 
        self.con = get_connection()  # الاتصال مع ملف الداتا 
        # (تم التصحيح: اضافة الاقواس)
        self.cursor = self.con.cursor()

    #----------------------------------------------------------------------------------------------------------
    # دوال القيت (جلب البيانات من الداتا بيز)
    #----------------------------------------------------------------------------------------------------------

    # 1. جلب بيانات الطالب
    def get_student_information(self, student_id):
        try: 
           # (تم التصحيح: اضافة علامة التنصيص في النهاية)
           sql_code = f"SELECT name, program, level FROM students WHERE student_id = '{student_id}'"
           self.cursor.execute(sql_code)
           info = self.cursor.fetchone()
           return info
        except sqlite3.Error as e:
            print("Database Error: Could not fetch info for", student_id, ". Details:", e)
            return None

    # 2. جلب الكورسات المكتملة للطالب
    def get_student_transcript(self, student_id):
       try:
        # (تم التصحيح: اضافة علامة التنصيص بعد student_id)
        sql_code = f"SELECT course_code FROM transcripts WHERE student_id = '{student_id}' AND grade != 'F'"
        self.cursor.execute(sql_code)
        
        Results_corses_completed = self.cursor.fetchall()
        corses_Completed = []
        for row in Results_corses_completed:
            corses_Completed.append(row[0])
        
        # (تم التصحيح: الـ return خارج اللوب)
        return corses_Completed
            
       except sqlite3.Error as e:
            print("Database Error: Could not fetch transcript for", student_id, ". Details:", e)
            return []

    # 3. جلب بيانات المادة
    def get_Course_Data(self, course_code):
        try:
            # (تم التصحيح: اضافة علامة التنصيص وازالة = الزائدة)
            sql_code = f"SELECT name, credits, max_capacity, lecture_hours, lab_hours FROM courses WHERE course_code = '{course_code}'"
            self.cursor.execute(sql_code)
            Corse_Data = self.cursor.fetchone()
            
            if not Corse_Data:
                return None
            
            # (تم التصحيح: اضافة علامة التنصيص)
            sql_code_pre = f"SELECT prereq_code FROM prerequisites WHERE course_code = '{course_code}'"
            self.cursor.execute(sql_code_pre)
            course_pre_results = self.cursor.fetchall()
            course_pre = []

            for row in course_pre_results:
                course_pre.append(row[0])

            return CourseData(code=course_code, name=Corse_Data[0], credits=Corse_Data[1], 
                              lecture_houre=Corse_Data[2], lap_hour=Corse_Data[3], 
                              capacity=Corse_Data[4], prerequisites=course_pre)
        except sqlite3.Error as e:
            # (تم التصحيح: استخدام course_code المتغير الصحيح)
            print("Database Error: Could not fetch Course data for", course_code, ". Details:", e)

    # 4. جلب خطة البرنامج
    def get_program_plan(self, program, levle, course_code):
        try:
            # (تم التصحيح: اضافة علامات التنصيص الناقصة)
            sql_code = f"SELECT course_code FROM program_plans WHERE program = '{program}' AND level = {levle} AND course_code = '{course_code}'"
            self.cursor.execute(sql_code)
            result = self.cursor.fetchone()
            return result
    
        except sqlite3.Error as e:
            print(f"Database Error: Failed to check program plan for {course_code}. {e}")
            return None

    # 5. جلب عدد التسجيل الحالي
    def get_current_regestion(self, course_code):
     try:
         # (تم التصحيح: اضافة علامة التنصيص في النهاية)
         sql_code = f"SELECT COUNT(r.student_id) FROM registrations r JOIN offerings o ON r.offering_id = o.offering_id WHERE o.course_code = '{course_code}'"
         self.cursor.execute(sql_code)
    
         Current_Regestion = self.cursor.fetchone()[0]
         return Current_Regestion
    
     except sqlite3.Error as e:
        print(f"Database Error: Failed to check enrollment for {course_code}. {e}")
        return 0
    
    # 6. جلب الجدول الزمني للمادة
    def get_schedule_for_course(self, selected_Corses_Code):
        all_time_blocks = []
        # هنا نفترض ان selected_Corses_Code هي لسته من اوبجكتات CourseData
        for course in selected_Corses_Code:
          try:
             # (تم التصحيح: اسم الجدول offerings جمع، واضافة علامة تنصيص)
             sql_code = f"SELECT day_of_week, start_time, end_time FROM offerings WHERE course_code = '{course.code}'"
             self.cursor.execute(sql_code)
             schedule_results = self.cursor.fetchall()
          except sqlite3.Error as e:
                print("DB Error: Failed to fetch offerings for", course.code , e)
                continue 
          
          for raw_block in schedule_results:
                day = raw_block[0]
                start_time_raw = raw_block[1]
                end_time_raw = raw_block[2]
                try:
                    # تحويل الوقت
                    start_time_parts = start_time_raw.split(':')
                    start_hours = int(start_time_parts[0])
                    start_minutes = int(start_time_parts[1])
                    processed_start_time_in_minutes = start_hours * 60 + start_minutes
                    
                    end_time_parts = end_time_raw.split(':')
                    end_hours = int(end_time_parts[0])
                    end_minutes = int(end_time_parts[1])
                    processed_end_time_in_minutes = end_hours * 60 + end_minutes
                    
                    all_time_blocks.append(TimeBlock(course.code, day, processed_start_time_in_minutes, processed_end_time_in_minutes))

                except ValueError:
                    print("Error parsing time format for course ", course.code)
                    continue

        return all_time_blocks

   #----------------------------------------------------------------------------------------------------------
   # دوال التشييك (التحقق من الشروط)
   #----------------------------------------------------------------------------------------------------------

    # 1. دالة التشييك على عدد الساعات
    def check_credit_hours(self, Corses_with_data):
        Min_credits = 12
        Max_credits = 18
        total_credits = 0
        for i in Corses_with_data:
            # (تم التصحيح: credits)
            total_credits += i.credits 
        
        if total_credits < Min_credits:
            return f"Total credit hours {total_credits} is less than minimum required {Min_credits}."
        if total_credits > Max_credits:
            return f"Total credit hours {total_credits} exceeds maximum allowed {Max_credits}."

        return True
        
    # 2. دالة التشييك على المتطلبات المسبقة
    def check_prerequisites(self, student_transcipt, Courses_with_data):
        Course_prerequisites = Courses_with_data.prerequisites
        if not Course_prerequisites:
            return True
        for pre in Course_prerequisites:
            if pre not in student_transcipt:
                return f"Cannot registor for {Courses_with_data.code}. Prerequisites {pre} not completed."
        return True

    # 3. دالة التشييك على خطة الطالب
    def check_plan(self, student_info, Courses_with_data):
        student_program = student_info[1]
        student_levle = student_info[2]
        Courses_Allowed = self.get_program_plan(student_program, student_levle, Courses_with_data.code)
        
        if Courses_Allowed is None:
            return "Cannot register:", Courses_with_data.code, "is not part of the standard plan."
        return True

    # 4. دالة التشييك على التعارض الزمني
    def check_conflict(self, selected_Corses_Code):
        all_schedule_blocks = self.get_schedule_for_course(selected_Corses_Code)
        for i in range(len(all_schedule_blocks)):
            for j in range(i+1, len(all_schedule_blocks)):
                block1 = all_schedule_blocks[i]
                block2 = all_schedule_blocks[j]
                
                # (تم التصحيح: اضافة شرط تطابق اليوم)
                if block1.day == block2.day:
                    if block1.start_tim < block2.end_tim and block2.start_tim < block1.end_tim:
                        return f"Time conflict detected: Course {block1.course_code} overlaps with {block2.course_code} on {block1.day}."
        return True 
        
    # 5. دالة التشييك على السعة
    def check_capacity(self, Corse_Data):
        max_capacity = Corse_Data.capacity
        course_code = Corse_Data.code
        # (تم التصحيح: اضافة الاقواس للاستدعاء)
        current_re = self.get_current_regestion(course_code)
    
        if current_re >= max_capacity:
            return f"Cannot register for {course_code}: Course capacity of {max_capacity} has been reached."
        return True

    #----------------------------------------------------------------------------------------------------------
    # دالة التحقق النهائية
    #----------------------------------------------------------------------------------------------------------

    def validate_schedule(self, Student_id, Selected_Corses_Code):
        try:
            student_transcipt = self.get_student_transcript(Student_id)
            student_info = self.get_student_information(Student_id)
            
            if student_info is None:
                return "Invalid Student ID"

            Courses_with_data = [] 
            for i in Selected_Corses_Code:
                course_data = self.get_Course_Data(i)
                if course_data is None:
                    return f"Course {i} does not exist."
                Courses_with_data.append(course_data)

            # تشييك credit hours (مرة واحدة)
            result = self.check_credit_hours(Courses_with_data)
            if result is not True:  
                return result

            # تشييك conflict (مرة واحدة للقائمة كاملة) - (تم التصحيح: اخراجها من اللوب)
            result = self.check_conflict(Courses_with_data)
            if result is not True: 
                return result

            # اللوب للفحوصات الفردية
            for Corse in Courses_with_data:
                # prerequisites
                result = self.check_prerequisites(student_transcipt, Corse)
                if result is not True: return result
                
                # plan
                result = self.check_plan(student_info, Corse)
                if result is not True: return result
                
                # capacity
                result = self.check_capacity(Corse)
                if result is not True: return result
            
            # (تم التصحيح: الارجاع النهائي خارج اللوب)
            return True

        except Exception as e:
            return f"System Error: {e}"
        
    def register_sudent(self, student_id, selected_courses_codes):
        try:
           for course_code in selected_courses_codes:
               # (تم التصحيح: اضافة علامات تنصيص)
               sql_code = f"SELECT offering_id FROM offerings WHERE course_code = '{course_code}'"
               self.cursor.execute(sql_code)
               result = self.cursor.fetchone()

               if result:
                   offering_id = result[0]
                   # (تم التصحيح: اضافة علامات تنصيص ل student_id)
                   sql_insert = f"INSERT INTO registrations (student_id, offering_id) VALUES ('{student_id}', {offering_id})"
                   self.cursor.execute(sql_insert)
               else:
                   print(f"Warning: No offering found for course {course_code}")
           
           self.con.commit()
           # (تم التصحيح: ارجاع True)
           return True
           
        except sqlite3.Error as e:
            self.con.rollback()
            return f"Database Error during registration: {e}"
